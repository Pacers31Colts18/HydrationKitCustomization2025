...
$NewGroups = Import-CSV -Path "$($sourcePath)\ADGroupList.csv"
Write-HYDLog -Message "Importing list of new groups"
...
foreach ($group in $NewGroups) {
    New-ADGroup -Name $group.SAMAccountName -Description $group.Description -Path $group.Path
    try {
        Get-ADGroup -Identity $group.Name
        Write-HYDLog "Group $($group.Name) created"
    }
    catch {
        Write-HYDLog "Failed to create group $($Group.Name)"
    }
}